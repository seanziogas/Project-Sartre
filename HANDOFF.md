...
- Learning speed 3 (`@sartre/learning`, Phase 4 pulled forward) — Thompson-sampling variant allocation from outcome events (guaranteed 5% observability floor, injectable RNG, mix-changes-only guardrail: new copy still enters via review), ICP recalibration (per-band conversion, inversion detection, disqualifier-rejecting-viable-accounts flag → proposals, never silent), and the baseline→delta engagement report (Layer 5 renewal artifact: data-health delta + approve-without-edit trend + delivery/spend, all computed from journal + feedback log).
- `@sartre/memory` (Layer 6 as code) — insight-node parse/lint (blessed statuses/sources, max 7 tags, attribution counting, concept-not-meeting filenames), wiki-link extraction + instance-scoped resolution (forward refs legal, attachments excluded), graph health (counts/status, orphans + well-connected %, emerging-tag threshold tracking, stale in-progress via lastTouched, synthesis gaps, Healthy/Warning/Needs Attention rating + markdown render), and LLM meeting ingestion (transcript → meeting file + 1–5 insight nodes; frontmatter assembled deterministically, model output validated + linted with retry, throws after exhaustion).

**Per Sean 2026-07-09: API credentials deprioritized — test with mock data.** Remaining: live connector clients (whenever credentials appear) + deployment wiring of module deps in the runner registry; wiring `apps/ops` to @sartre/db (currently FileRunStore); live-model eval runs; **Hologram shadow-run staged and one command from the number** (`tools/shadow-hologram/` — fixtures built from the reference copy into gitignored shadow-runs/, plumbing verified with --fake; live run needs `ANTHROPIC_API_KEY` or `ant auth login`, then `node tools/shadow-hologram/run.mjs`); Onboarding Week codified at docs/onboarding-week.md.
