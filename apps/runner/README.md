...
The deployment module keeps credential-bearing connector construction outside the platform registry. It receives `{ db, brains, connections, tools }`; `brains` loads active, human-approved documents and typed config from the requested client instance. `connections` can list or resolve only the requested client's active connections. `tools` constructs tenant-scoped clients for CRM (Salesforce, HubSpot, Attio), enrichment (Clay), collaboration (Slack, Teams), email (Gmail, Microsoft Graph), transcripts (Fathom, Gong), sequencers (Smartlead, Instantly, Outreach, Salesloft, Apollo, HeyReach, lemlist, Mailshake), audiences (LinkedIn Ads), warehouses (Snowflake, BigQuery), intent (6sense, G2, Clearbit, Koala, Bombora), and inbound sources (Qualified, LinkedIn Lead Gen, Typeform, Chili Piper). Credentials are decrypted only by an explicit execution-time call and are not cached. The runner owns and injects the locked production model adapter separately.
