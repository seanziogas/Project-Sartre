...
Deployment still requires an identity proxy, real access grants, `DATABASE_URL`, and—only for live copilot requests—`ANTHROPIC_API_KEY`. CI uses scripted LLM fakes and contains no credentials or client data.
