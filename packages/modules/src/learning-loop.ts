import { HumanActionEvent } from '@sartre/core'
...
import type { TuningProposal } from '@sartre/learning'
...
interface LearningReview {
  ...
  evalFailures: { proposal: TuningProposal; detail: string }[]
    ...
    steps: [
      ...
      },
      {
        ...
        run: async (ctx) => {
          ...
          const evalFailures = evaluated
            .filter((proposal) => !proposal.evalResult.pass)
            .map(({ evalResult, ...proposal }) => ({ proposal, detail: evalResult.detail }))
...
function renderDraftTuningReport(
  ...
  return [
    '---',
    'brain_doc: tuning_proposal',
    `client: ${clientId}`,
    'status: draft',
    `updated: ${date}`,
    `sources: [${sourceEventIds.map((id) => `"feedback-event:${id}"`).join(', ')}]`,
    'approved_by: ""',
    '---',
    '',
    renderTuningReport(clientId, date, proposals),
  ].join('\n')
}
