...
| `learned/` | ✅ (dir) | **Layer 8 writes here, humans approve:** `exemplars/` (`brain_doc: exemplar`) and evaluated tuning/allocation/calibration drafts (`brain_doc: learning-proposal`), plus `thresholds.yaml`; machine use still requires active status and `approved_by` |
